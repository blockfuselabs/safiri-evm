...
const ussdAccess = async (req, res) => {
    ...
    // More complex logics
        ...
        if (parseInt(array[0]) == 4 && array.length === 2) {
            ...
            if (!userExist) {
                ...
                const tokenContract = new ethers.Contract(USDT_CONTRACT_ADDRESS, ERC20_ABI, ethProvider);
                const userAddress = userExist.walletAddress;

                const userBalance = await tokenContract.balanceOf(userAddress);

                if (parseFloat(amount) > parseFloat(userBalance)) {
                    response = 'END Insufficient crypto balance';
                } else {
                    try {
                        const { getQuoteOut, transferOut } = require('./oneRampService');

                        // Step 1: Get quote from OneRamp
                        const quote = await getQuoteOut({
                            fiatType: 'NGN',
                            cryptoType: 'cUSD',
                            cryptoAmount: amount,
                            country: 'NG',
                        ...
                        // Step 2: Perform off-ramp transfer
                        const transfer = await transferOut({
                            quoteId: quote.quoteId,
                            bank: {
                                code: userExist.bankCode,
                                accountNumber: userExist.accountNumber,
                                accountName: userExist.accountName
                            },
                            userDetails: {
                                name: userExist.accountName,
                                country: 'NG',
                                phone: phoneNumber,
                                address: 'NG',
                        ...
                        await Transaction.create({
                            ...
                            txHash: transfer.transactionHash || 'oneramp-offramp',
                            amount: parseFloat(amount),
                            serviceBeneficiary: userExist.accountName,
                            date: new Date()
                        });

                        // Step 4: USSD + SMS confirmation
                        response = `END Transfer of ${amount} USDT initiated. You'll receive NGN in ${userExist.accountNumber}.`;
                        sendSMS(phoneNumber, `Safiri: Your transfer of ${amount} USDT is being processed. NGN will be deposited to ${userExist.accountNumber}.`);

                    } catch (error) {
                        console.error('Offramp error:', error.message);
                        response = 'END Offramp failed. Please try again later.';
                    }
                }
            }
        }
